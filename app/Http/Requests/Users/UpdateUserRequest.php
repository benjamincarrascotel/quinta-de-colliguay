...
class UpdateUserRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $existingUser = $user instanceof User ? $user : User::findOrFail($user);
        ...
        if ($requiresCompany) {
            ...
            // The User can't change his own company
            if ($existingUser->company_id !== null) {
                $companyRules[] = function ($attribute, $value, $fail) use ($existingUser) {
                    if ((int) $value !== (int) $existingUser->company_id) {
                        $fail('No se puede transferir un usuario a otra compañía.');
                    }
                };
            }
        } else {
